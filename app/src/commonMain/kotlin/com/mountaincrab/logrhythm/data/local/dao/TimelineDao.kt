 ...
 *
 * The entry tables can't share a single `SELECT *` (their columns differ), but they
...
interface TimelineDao {
    /**
     * Returns up to [limit] `occurredAt` values (across poop/food/note/medication) strictly older than
    ...
    @Query(
        ...
        SELECT occurredAt FROM (
            ...
            UNION ALL
            SELECT occurredAt FROM medication_entries WHERE isDeleted = 0 AND profileId = :profileId AND occurredAt < :before
        )
